...
export async function GET() {
  try {
    ...
    const { data: claims } = await supabase
      .from('day_claims')
      .select('epoch_day, claimer_wallet, incentive_lamports, image_url, link_url, farcaster_username, farcaster_pfp_url, moderation_status')
    ...
    for (let d = windowStart; d <= windowEnd; d++) {
      ...
      days.push({
        ...
        linkUrl: claim && isApproved ? claim.link_url || null : null,
