...
const VALID_TABS = new Set<Tab>(['home', 'mint', 'claim']);
    ...
    fetch('/api/calendar')
      ...
      .then((data) => {
        ...
        if (todayClaim?.claimed && todayClaim?.moderationStatus === 'approved') {
          ...
          if (baseUrl) {
            const imgUrl = `${baseUrl}/storage/v1/object/public/day-images/day-${todayClaim.epochDay}.png`;
            // Verify image exists before setting
            const img = new Image();
            img.onload = () => setTodayImageUrl(imgUrl);
            img.src = imgUrl;
          }
        }
      })
